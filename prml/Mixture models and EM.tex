...
\begin{itemize}
	\item[Nonlinear optimization] conjugate gradients algorithm in M step
	\item[expectation conditional maximization(ECM)] makint several constrained optimizations within each M step.
\end{itemize}

Generalize the E step by performing a partial optimization of $\mathcal{L}(q,\vec{\theta})$ with respect to $q(\vec{Z})$.
